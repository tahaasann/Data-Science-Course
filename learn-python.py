...
#-------------------------------------------------
# ÖRNEK ÖZELLİKLERİ
# Herbir örneğin kendi içerisinde değişen özelliklerden oluşabilmesini sağlayacağız. 

class VeriBilimci(): # Sınıf özellikleriyle, örnek özelliklerini aynı isimlerle yapmak doğru bir yaklaşım değildir. Biraz daha farklı isimlerle yapmak önemlidir.
...
class VeriBilimci():
    ...
    def __init__(self):
        self.bildigi_diller = []
        self.bolum = ''
    def dil_ekle(self, yeni_dil):# self'i, örnekleri temsil edecek ve fonksiyonu örnekler için kullanacak.
...
VeriBilimci.dil_ekle("R") # TypeError, böyle bir tanımlama yapmadık
